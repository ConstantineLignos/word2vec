...
void *TrainModelThread(void *id) {
  ...
  while (1) {
    if (word_count - last_word_count > 10000) {
      ...
      if ((debug_mode > 1)) {
        now = clock();
        printf("%cAlpha: %f  Progress: %.2f%%  Words/thread/sec: %.2fk  ", 13, alpha, word_count_actual / (real) (train_words + 1) * 100,
...
int main(int argc, char **argv) {
  ...
  if ((i = ArgPos((char *) "-load-weights", argc, argv)) > 0)
    strcpy(load_weights_file, argv[i + 1]);
